public({
  ...
  test_that("Cache gets set on cachedPOST", {
    ...
    with_fake_http({
      ...
      expect_POST(
        b <<- cachedPOST("https://app.crunch.io/api/",
          body = '{"user":"me"}'
        ),
  ...
  without_internet({
    ...
    test_that("But uncached() prevents reading from the cache", {
      uncached({
        expect_POST(
          cachedPOST("https://app.crunch.io/api/"),
          "https://app.crunch.io/api/"
        )
        expect_POST(
          cachedPOST("https://app.crunch.io/api/",
    ...
    test_that("GETs don't read from cachedPOST cache", {
      expect_GET(
        uncached(GET("https://app.crunch.io/api/")),
        "https://app.crunch.io/api/"
      )
    })
    test_that("And POSTs with different payloads don't read the wrong cache", {
      expect_POST(
        cachedPOST("https://app.crunch.io/api/", body = "wrong"),
